...
public class TrimRawBlockData extends DoFn<PubsubMessage, String> {
    public TrimRawBlockData() {}
    ...
    public void processElement(ProcessContext ctx, OutputReceiver<String> r) 
        ...
        try {
            ...
            logger.debug("Extracted ethereum block raw message: " + payload);

            JsonNode json = mapper.readTree(payload);
            ObjectNode jsonRoot = (ObjectNode) json;

            // Optional: add timestamp in Elasticsearch's native tongue
            jsonRoot.put("@timestamp", jsonRoot.get("timestamp").asLong());
            // jsonRoot.remove("timestamp"); // leave for BigQuery
    ...
    // Instantiate Logger
    private static final Logger logger = LoggerFactory.getLogger(TrimRawBlockData.class);
}
